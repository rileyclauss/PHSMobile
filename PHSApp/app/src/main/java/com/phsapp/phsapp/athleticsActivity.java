...
public class athleticsActivity extends AppCompatActivity implements View.OnClickListener, OnRssLoadListener {
    ...
    private void loadFeeds() {
        String[] urlArr = {"http://pennant.phmschools.org/feed/"}; //Gives the RSS parser something to read
        new RssReader(athleticsActivity.this).showDialog(false).urls(urlArr).parse(this);  //Begins RSS parser. Success -> onSuccess. Failure -> onFailure
    }
    ...
    public void onSuccess(List<RssItem> rssItems) {
        ...
        for (int i = 0;i<10;i++) {
            if (rssItems.get(i).getDescription().contains("uploads")) {  //'uploads' is the guarantee of an imgLink in the description
                x = rssItems.get(i).getDescription().indexOf(" src");    //src is found before the imgLink
                x += 6;                                                 //used to bring x to the beginning of the link
                y = rssItems.get(i).getDescription().indexOf(" alt");   //alt is after
                y -= 1;                                                  //bring y to the end of the link
                //constructor for athleticsEntries, rssItems is autogenerated by RSS reader,  substring divides the desc for the img link
    ...
    @Override
    public void onFailure(String message) {   //When the RSS Reader fails,
